...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = getContentBySlug("alternatives", slug);
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://bknddevelopment.com/alternatives/${slug}`,
...
export default async function AlternativesArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const post = getContentBySlug("alternatives", slug);
      ...
      <main className="min-h-screen bg-white pt-20">
        <article className="max-w-4xl mx-auto px-6 py-16">
          ...
          <nav className="text-sm text-[#86868B] mb-8">
            ...
            <a href="/alternatives" className="hover:text-[#1D1D1F] transition-colors">
              Alternatives
            </a>
            <span className="mx-2">/</span>
