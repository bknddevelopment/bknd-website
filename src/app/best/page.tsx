...
export const metadata: Metadata = {
  title: "Best-Of Roundups | BKND Development",
  description:
    "Curated picks for the tools and platforms that matter.",
};

export default function BestHub() {
  const posts = getContentByCategory("best-of");
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <AnimateOnScroll key={post.slug}>
                  <Link href={`/best/${post.slug}`} className="group block">
                    {post.featuredImage && (
                      <img
